...
// 
// addr/$address/transactions?page=$page&offset=$offset&sort=asc
const personalTransaction = (req, res, next) => {
	...
	gasTrackerClient(`addr/${address}/transactions?page=${pageNum}&offset=${PAGESIZE}`)
		.then(result => {
		  return responseSuccess(res, result.items);
		})
		.catch(e => {
